...
### Added
- **`--session-idle-timeout` / `GIZMOSQL_SESSION_IDLE_TIMEOUT`.** Evict idle client sessions after N seconds without user SQL (DuckDB backend). `0` = off (default). Background sweeper reuses the existing session-removal path (client sees `Unauthenticated` / session not found — evicted). Same audience as DuckDB Core session management. Closes [#141](https://github.com/gizmodata/gizmosql/issues/141).
