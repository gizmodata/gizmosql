...
- **`--session-idle-timeout` / `GIZMOSQL_SESSION_IDLE_TIMEOUT`.** Evict idle client sessions after N seconds without user SQL (DuckDB backend). `0` = off (default). Background sweeper reuses the existing session-removal path (client sees `Unauthenticated` / session not found — evicted). If an idle session is evicted, any work that was started but not committed is undone. Same audience as DuckDB Core session management. Closes [#141](https://github.com/gizmodata/gizmosql/issues/141).
...
- **Forced shutdown (grace-period elapsed / second signal) now actually stops the server promptly.** Previously a "force" only called gRPC's `Shutdown()`, which **waits** for synchronous query handlers to finish rather than preempting them — so the server still blocked until the straggler query completed (defeating the whole point of the grace cap). The forced path now **interrupts every in-flight query** (the same mechanism as `KILL SESSION`) before shutting the transport down, so normal queries abort within a fraction of a second and the server stops at the cap. (Operations that ignore DuckDB's interrupt flag, such as `sleep_ms()`, still run to completion — only `SIGKILL` stops those.) Adds regression tests for both the grace-period-elapsed and second-signal force paths.

## [1.31.1] - 2026-06-18

### Added

- **`gizmosql_server --print-duckdb-version`** — prints the version of the linked DuckDB library (e.g. `v1.5.4`) and exits. Handy for scripts/CI and for telling the stable and LTS channels apart at a glance (they pin different DuckDB releases). See [LTS Channel](https://docs.gizmosql.com/lts_channel.md).
...
- **Admin-command gating: dangerous instance-level commands now require the `admin` role.** A rudimentary, always-on guardrail ahead of the full GizmoSQL 2.0 RBAC model. For **non-admin** sessions, the server now rejects (with a permission error) `ATTACH`/`DETACH`, `SET GLOBAL`/`RESET GLOBAL` (and dangerous bare `SET` of global-only settings like `memory_limit`/`threads`/`enable_external_access`), `INSTALL`/`LOAD` extensions, `CHECKPOINT`, `EXPORT DATABASE`/`IMPORT DATABASE` (any destination — full-database egress/ingress), `COPY` to/from the **local** filesystem, the `read_*`/`glob`/`sniff_csv` table functions reading the **local** filesystem (at any nesting), local-path replacement scans (`SELECT * FROM '/path'`), `CREATE SECRET`/`DROP SECRET` (all variants — secrets hold credentials), and `duckdb_secrets()`. Remote object-storage/HTTP paths (`s3://`, `gs://`, `https://`, …) remain allowed for `read_*`/`COPY` (bounded reads/writes), but **not** for `EXPORT`/`IMPORT DATABASE`. Detection is parser-based (robust to whitespace/comments/case/nesting). **Basic-auth (username/password) users are unaffected — they are always `admin`** — so this only impacts token/SSO deployments that mint non-admin roles. Without it, a non-admin token user could detach a DuckLake catalog, read arbitrary local files, or change global server settings. See [Admin Command Gating](https://docs.gizmosql.com/admin_command_gating.md).
