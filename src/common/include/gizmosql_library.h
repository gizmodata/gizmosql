 ...
 * @param enable_instrumentation [Enterprise] Whether to enable session instrumentation (tracking instances, sessions, SQL statements). Default is false. If not set, uses env var GIZMOSQL_ENABLE_INSTRUMENTATION (1/true to enable). Requires valid enterprise license.
 ...
 * @param allow_cross_instance_tokens Allow tokens issued by other server instances (with the same secret key) to be accepted. Default is false (strict mode). If not set, uses env var GIZMOSQL_ALLOW_CROSS_INSTANCE_TOKENS (1/true to enable). Useful for load-balanced deployments where clients may reconnect to different instances.
 ...
 * @param otel_enabled Enable OpenTelemetry instrumentation. Default is false. If not set, uses env var GIZMOSQL_OTEL_ENABLED (1/true to enable).
...
int RunFlightSQLServer(
    ...
    bool allow_cross_instance_tokens = false,
    std::string oauth_client_id = "",
    std::string oauth_client_secret = "",
    std::string oauth_scopes = "",
    int oauth_port = 0,
    std::string oauth_base_url = "",
    bool oauth_disable_tls = false,
    bool otel_enabled = false, std::string otel_exporter = "",
